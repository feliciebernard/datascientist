...
def how_many(journalists)
  number = journalists.length
  puts "Il y a #{number} handle"
  puts gets.chomp
end

#Quel est le handle le plus court de cette liste ?
def smaller_name(journalists)
  characters_number_min = journalists.min {|a, b| a.length <=> b.length}
  puts "#{characters_number_min} est l'handle le plus petit."
  characters_number_max = journalists.max {|a, b| a.length <=> b.length}
  puts "(Et l'handle le plus grand, c'est #{characters_number_max} ! :D)"
  puts gets.chomp
end

#Combien y a t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def five_characters(journalists)
characters_five = journalists.count {|a| a.size == 6}
puts "Il y a #{characters_five} handle contenant 5 caractères."
puts gets.chomp
end

#Combien commencent par une majuscule (première lettre juste après le @) ?
def uppercase(journalists)
  uppercase_at_the_beginning = journalists.count {|a| a =~ /^@[A-Z]/}
  puts "Il y a #{uppercase_at_the_beginning} handle qui commencent par une majuscule."
  puts gets.chomp
end
...
def hight_if_handle(journalists)
  hight = journalists.sort_by! {|a| a.size}
  puts hight
  puts "(C'était la liste de handle rangée par taille de caractère :)"
  puts gets.chomp
end

#Quelle est la position dans l'array de la personne @epenser ?
def witch_position(journalists)
  position = journalists.index("@epenser")
  puts "La personne @epenser se situe en position n°#{position} de l'array"
  puts gets.chomp
end
...
def repartition(journalists)
  ...
  number_of_caracters = 0
    32.times do |a|
    characters = journalists.count {|a| a.size == handle}
    handle += 1
    number_of_caracters += 1
    puts "Il y a #{number_of_caracters} caractères dans #{characters} handle."
    end
end
...
def perform
journalists=handle
how_many(journalists)
smaller_name(journalists)
five_characters(journalists)
uppercase(journalists)
alphabetique_order(journalists)
hight_if_handle(journalists)
witch_position(journalists)
